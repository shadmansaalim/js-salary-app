...
function addRecordHandler() {
  ...
  if (!name || !salary || (typeof name != 'string') || isNaN(salary)) {
    showDataError();
    return;
  }
  clearFields();
...
function displayLastItemDialog(lastItem) {
...
const showDataError = () => {
  const recordBtn = document.getElementById('AddRecord');
  recordBtn.setAttribute("data-toggle", "modal");
  recordBtn.setAttribute("data-target", "#invalidRecord");
  return;
};
